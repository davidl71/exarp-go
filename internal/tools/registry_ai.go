...
func registerAITools(server framework.MCPServer) error {
	...
	if err := server.RegisterTool(
		"text_generate",
		"[HINT: provider=fm|ollama|insight|mlx|localai|gateway|auto. Unified text generation across all LLM backends. Single entry point for generate-text. task_type enables auto model selection.]",
		framework.ToolSchema{
			Type: "object",
			Properties: map[string]interface{}{
