...
| **ModelRouter / provider=auto** | Optionally add a `ModelType` (e.g. `ModelGateway`) and have `ResolveModelForTask` or the router select it when a gateway is configured and cost/quality suggests using it. Alternatively keep gateway as an explicit `provider=gateway` only and leave `auto` for local FM/Ollama/MLX. |
...
| **TextGenerator interface** | `Supported() bool` + `Generate(ctx, prompt, maxTokens, temperature) (string, error)`. LocalAI, FM, Ollama, MLX, llamacpp implement it. | New gateway provider implements `TextGenerator`; `text_generate` switch adds `case "gateway": gen = DefaultGatewayProvider()`. Same as LocalAI. |
