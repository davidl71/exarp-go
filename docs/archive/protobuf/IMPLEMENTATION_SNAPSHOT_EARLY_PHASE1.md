# Early protobuf implementation snapshot (Phase 1 task metadata)

> **Archived 2026-03-30.** Superseded for current truth by [PROTOBUF_IMPLEMENTATION_STATUS.md](../../PROTOBUF_IMPLEMENTATION_STATUS.md). This file is a **frozen milestone** from the first Todo2 metadata protobuf rollout (performance numbers and file lists reflect that era).
...
- **Database migration:**
  - `migrations/003_add_protobuf_support.sql` - Added `metadata_protobuf` (BLOB) and `metadata_format` (TEXT) columns
...
- `migrations/003_add_protobuf_support.sql`
- `buf.yaml`
