...
| **Report insight** | Report/scorecard AI blurbs (long-form text). | `DefaultReportInsight()` / FM chain; `internal/tools/insight_provider.go`, `report_insights.go`. *(The former `mlx` MCP tool and bridge were removed.)* |
...
- **Unified entry:** `text_generate` tool with `provider: fm | ollama | insight | localai | gateway | auto`.
...
- **Current:** Custom provider abstraction (FM, Ollama, LocalAI, gateway) and MCP tools; no chain/agent framework.
...
- **Usefulness for exarp-go:**
  ...
  - **Low** for **local-only** (Apple FM, Ollama): those backends use different tokenization; the current ratio is a reasonable heuristic.
...
- **Would tiktoken help?** Yes, when the consumer is an **OpenAI or OpenAI-compatible** API. It would:
  ...
  - For **local-only** (FM, Ollama), the ratio remains sufficient; tiktoken is optional.
...
| Cloud / self-hosted | Local (FM, Ollama) + optional LocalAI + gateway | Optional cloud SDKs later. |
...
**Relation to exarp-go:** We use **Apple FM** (Swift/CGO). **mlx-swift-lm** remains relevant for Swift apps that run MLX models directly; exarp-go no longer exposes an MCP `mlx` tool or Python MLX bridge.
