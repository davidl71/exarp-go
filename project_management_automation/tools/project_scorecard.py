...
Go delegation: For Go projects (go.mod present), scorecard is generated by exarp-go
report(action=scorecard); Python falls back to full implementation for non-Go projects.
"""
...
def _is_go_project(project_root: Path) -> bool:
    """Return True if project root has go.mod (Go project)."""
    return (project_root / "go.mod").exists()


def _get_exarp_go_binary(project_root: Path) -> str:
    """Return path to exarp-go binary: bin/exarp-go in project or 'exarp-go' in PATH."""
    bin_exarp = project_root / "bin" / "exarp-go"
    if bin_exarp.exists():
        return str(bin_exarp)
    return "exarp-go"


def _call_go_scorecard_json(project_root: Path) -> Optional[dict[str, Any]]:
    """
    Call exarp-go report(action=scorecard, output_format=json) and return parsed dict.
    Returns None on failure or if not a Go project.
    """
    if not _is_go_project(project_root):
        return None
    binary = _get_exarp_go_binary(project_root)
    args_json = json.dumps({"action": "scorecard", "output_format": "json"})
    try:
        proc = subprocess.run(
            [binary, "-tool", "report", "-args", args_json],
        ...
        if proc.returncode != 0:
            scorecard_logger.debug("exarp-go scorecard failed: %s", proc.stderr)
            return None
        out = (proc.stdout or "").strip()
        if not out:
            return None
        # CLI prints "Executing tool: ...\nArguments: ...\n\nResult:\n{json}"
        if "Result:\n" in out:
            out = out.split("Result:\n", 1)[1].strip()
        return json.loads(out)
    except (subprocess.SubprocessError, json.JSONDecodeError, FileNotFoundError) as e:
        scorecard_logger.debug("exarp-go scorecard call failed: %s", e)
        return None
...
def generate_project_scorecard(
    ...
    # Go project: delegate to exarp-go report(scorecard)
    if _is_go_project(project_root):
        ...
        if data:
            ...
            if output_format == "json":
                ...
                args_json = json.dumps({"action": "scorecard", "output_format": output_format})
                try:
                    proc = subprocess.run(
                    ...
                    formatted_output = (proc.stdout or "").strip() if proc.returncode == 0 else json.dumps(result, indent=2)
                except Exception:
            ...
            if output_path:
                ...
                Path(output_path).write_text(formatted_output)
                result['output_file'] = str(output_path)
            _save_scorecard_memory(result)
            return result
