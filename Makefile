...
.PHONY: test test-watch test-go test-go-fast test-go-race test-go-verbose test-go-parallel test-go-tools-short test-real-models test-ollama test-coverage test-html test-tools test-cli test-cli-list test-cli-tool test-cli-test test-mcp-stdio
...
.PHONY: ansible-check ansible-dev ansible-list ansible-galaxy ansible-docker-smoke ansible-linters
...
ANSIBLE_CACHE_ROOT := $(REPO_ROOT)/.cache/ansible
ANSIBLE_LOCAL_TEMP := $(ANSIBLE_CACHE_ROOT)/tmp
ANSIBLE_REMOTE_TMP := $(ANSIBLE_CACHE_ROOT)/remote-tmp
ANSIBLE_GALAXY_CACHE_DIR := $(ANSIBLE_CACHE_ROOT)/galaxy-cache
ANSIBLE_HOME_DIR := $(ANSIBLE_CACHE_ROOT)/home
...
# Default Go caches to repo-local paths so tests and builds work inside sandboxes/CI
# without needing access to user-level cache directories.
...
config: ## Configure build system (detect available tools)
	...
	@if command -v $(GO) >/dev/null 2>&1 || [ -x "$(GO)" ]; then \
		...
		if [ $$go_major -gt 1 ] || ([ $$go_major -eq 1 ] && [ $$go_minor -ge 20 ]); then \
			...
			echo "$(GREEN)✅ Found: Go $$go_version (1.20+)$(NC)"; \
		else \
			echo "HAVE_GO_1_20_PLUS = 0" >> .make.config; \
			echo "GO_VERSION = $$go_version" >> .make.config; \
			echo "$(YELLOW)⚠️  Found: Go $$go_version (1.20+ required)$(NC)"; \
		fi; \
	else \
		echo "HAVE_GO_1_20_PLUS = 0" >> .make.config; \
		echo "GO_VERSION = unknown" >> .make.config; \
		echo "$(RED)❌ Not found: Go$(NC)"; \
	fi
	@# Detect CGO support
	@if command -v gcc >/dev/null 2>&1 || command -v clang >/dev/null 2>&1 || command -v cc >/dev/null 2>&1; then \
		echo "HAVE_CGO = 1" >> .make.config; \
		echo "$(GREEN)✅ Found: C compiler (CGO enabled)$(NC)"; \
	else \
		echo "HAVE_CGO = 0" >> .make.config; \
		echo "$(YELLOW)⚠️  Not found: C compiler (CGO disabled - Apple FM unavailable)$(NC)"; \
	fi
	@# Detect SQLite3
	@if command -v sqlite3 >/dev/null 2>&1; then \
		echo "HAVE_SQLITE3 = 1" >> .make.config; \
		echo "$(GREEN)✅ Found: sqlite3$(NC)"; \
	else \
		echo "HAVE_SQLITE3 = 0" >> .make.config; \
		echo "$(YELLOW)⚠️  Not found: sqlite3 (optional - for Todo2 database)$(NC)"; \
	fi
	@# Detect git
	@if command -v git >/dev/null 2>&1; then \
		echo "HAVE_GIT = 1" >> .make.config; \
		echo "$(GREEN)✅ Found: git$(NC)"; \
	else \
...
dev-cycle: go-fmt test-go-fast ## Quick development cycle (format, test, ready for commit)
	@echo "$(GREEN)✅ Development cycle complete - ready for commit$(NC)"
...
help: ## Show this help message
	...
	@if [ -f .make.config ]; then \
		...
		if [ -n "$$(grep 'HAVE_GO_1_20_PLUS = 1' .make.config 2>/dev/null)" ]; then \
			...
			echo "  $(GREEN)✅ Go $$go_ver$(NC)"; \
		else \
			echo "  $(YELLOW)⚠️  Go (1.20+ required)$(NC)"; \
		fi; \
		if [ -n "$$(grep 'HAVE_CGO = 1' .make.config 2>/dev/null)" ]; then \
			echo "  $(GREEN)✅ CGO (Apple FM support)$(NC)"; \
		else \
			echo "  $(YELLOW)⚠️  CGO (Apple FM unavailable)$(NC)"; \
		fi; \
...
test-go-race: ## Run Go tests with race detector (slower; stability / concurrency guard)
	@echo "$(BLUE)Running Go tests with -race (CGO_ENABLED=0, short mode)...$(NC)"
	@CGO_ENABLED=0 $(GO) test ./... -race -short -parallel 4 -count=1 || \
	 (echo "$(RED)❌ Race detector tests failed$(NC)" && exit 1)
...
delete-expired-archive: ## Delete expired docs/archive files per ARCHIVE_RETENTION_POLICY (dry-run by default; DRY_RUN=0 to actually delete)
	@if [ "$(DRY_RUN)" = "0" ]; then \
		$(GO) run ./scripts/delete_expired_archive.go -dry-run=false -project-root=. ; \
	else \
		$(GO) run ./scripts/delete_expired_archive.go -dry-run=true -project-root=. ; \
	fi
...
queue-dispatcher: build ## Run wave dispatcher (requires REDIS_ADDR)
	@if [ -z "$$REDIS_ADDR" ]; then \
		echo "$(YELLOW)⚠️  Set REDIS_ADDR (e.g. export REDIS_ADDR=127.0.0.1:6379)$(NC)"; \
		exit 1; \
	fi
...
tag-build-ok: build ## Tag current commit as last known-good build (overwrites build-ok); use: git diff build-ok, git log build-ok..HEAD
	@git tag -f build-ok && echo "$(GREEN)✅ Tagged current commit as build-ok$(NC)" && echo "  Compare: git diff build-ok  |  See breakage: git log build-ok..HEAD --oneline"
...
lint-shellcheck: ## Run shellcheck on scripts (requires shellcheck)
	@if ! command -v shellcheck >/dev/null 2>&1; then \
		if [ -n "$$CI" ]; then \
			echo "$(RED)shellcheck not found. Install it in CI before running lint-shellcheck$(NC)"; \
			exit 1; \
		else \
			echo "$(YELLOW)shellcheck not found; skipping local shell lint. Install with 'make ansible-linters' or your package manager.$(NC)"; \
		fi; \
	else \
		shellcheck -x scripts/*.sh ansible/run-dev-setup.sh && echo "$(GREEN)✅ shellcheck done$(NC)"; \
	fi

lint-yaml: ## Run yamllint on YAML/Ansible files (requires yamllint)
	@if ! command -v yamllint >/dev/null 2>&1; then \
		if [ -n "$$CI" ]; then \
			echo "$(RED)yamllint not found. Install it in CI before running lint-yaml$(NC)"; \
			exit 1; \
		else \
			echo "$(YELLOW)yamllint not found; skipping local YAML lint. Install with 'make ansible-linters' or your package manager.$(NC)"; \
		fi; \
	else \
		yamllint -f standard .github ansible && echo "$(GREEN)✅ yamllint done$(NC)"; \
	fi

lint-ansible: ## Run ansible-lint on playbooks/roles (auto-installs community.general from requirements.yml)
	@if ! command -v ansible-lint >/dev/null 2>&1; then \
		if [ -n "$$CI" ]; then \
			echo "$(RED)ansible-lint not found. Install it in CI before running lint-ansible$(NC)"; \
			exit 1; \
		else \
			echo "$(YELLOW)ansible-lint not found; skipping local Ansible lint. Install with 'make ansible-linters' or your package manager.$(NC)"; \
		fi; \
	else \
		mkdir -p "$(ANSIBLE_LOCAL_TEMP)" "$(ANSIBLE_REMOTE_TMP)" "$(ANSIBLE_GALAXY_CACHE_DIR)" "$(ANSIBLE_HOME_DIR)" && \
